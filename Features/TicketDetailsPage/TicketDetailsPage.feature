...
Feature: Ticket Details page
    ...
    Scenario Outline: Title,last updated time , Last updated by , assigned to tab ,Description , Conversation in the ticket details page
        ...
        When "Admin" clicks on the ticket number in ticket console
        Then "admin" verifies the page must have "Similar Tickets"
        Then "admin" verifies make sure the Assigned to tab is present
        And "Admin" gets the short description
        Then "admin" verifies the ticket title is same as Short description in ServiceNow "<TicketNumber>"
        Then "admin" verifies the description is same as in the Description in servicenow "<TicketNumber>"
        Then "admin" verifies make sure the conversation is same as in the comments in the activity section in servicenow "<TicketNumber>"
