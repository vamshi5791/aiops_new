...
export class ApiKibana {
    ...
    //Bussiness Time 
    async businessTime(ProjectId = 1519, alertId: string) {
        const kibenaApiUrl = KibanaAPI_URL + '?path=alerts_' + ProjectId + '_1/_search&method=POST';
        await fetch(kibenaApiUrl, {
            method: 'POST',
            headers: {
                "Content-type": "application/json;charset=UTF-8",
                "Authorization": `Basic ${base64.encode(`${Kibana_UserID}:${Kibana_Password}`)}`,
            ...
            body: '{"query": {"match": {"alertID": "' + alertId + '"}}}'
        }).then(response => response.json())
            .then(resp => {
                var JsonAlertData = resp.hits.hits
                businessTime = JsonAlertData[0]._source
